...
const AddItem = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const res = await axios.post('/api/items/create', data, {
        headers: {
    ...
    <div className="text-center my-10 ">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80">
          <div className="bg-e w-full max-w-2xl p-6 rounded-lg shadow-lg relative">
            <button
  ...
  <input
    ...
    className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-2 focus:ring-green-400"
  ...
  <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
    <select
      name="category"
      className="bg-gray-800 border border-gray-700 px-4 py-2 rounded-md focus:outline-none focus:ring-2 focus:ring-green-400"
      onChange={handleChange}
      required
    >
  ...
  <button
    type="submit"
    className="w-full py-2 px-4 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-md transition-all duration-200"
    disabled={loading}
  >
